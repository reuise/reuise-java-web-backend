...
import dev.reuise.web.core.table.AbstractWebTableHeaderOptions;
...
// Option: Debug - CORE
// Option: DebugId - CORE
public abstract class AbstractWebBackendTableHeaderOptions<S extends AbstractWebBackendTableHeaderOptions<S>> extends AbstractWebTableHeaderOptions<S> implements WebBackendTableHeaderOptions {
    ...
    protected ComponentCreator<? extends CoreTableHeaderRow, ? extends CoreTableHeaderRowOptions> rowCreator;

    protected AbstractWebBackendTableHeaderOptions() {
        this(null);
    }

    protected AbstractWebBackendTableHeaderOptions(WebBackendParentComponentOptions baseOptions) {
    ...
    public boolean onPreInitialize() {
        ...
        setTagName("thead");
        setBaseStyleClass("reuise-table_header");
        return true;
    }
    ...
    @Override
    public WebBackendTableHeader getComponent() {
        return ((WebBackendTableHeader) (baseComponentPart.getComponent()));
    }

    @Override
    public void merge(CoreComponentOptions other) {
        baseComponentPart.merge(other);
        if (((WebBackendTableHeaderOptions) (other)) instanceof WebBackendTableHeaderOptions) {
            WebBackendTableHeaderOptions otherWebBackendTableHeaderOptions = ((WebBackendTableHeaderOptions) (other));
            CoreTableHeaderRowOptions otherRowOptions = otherWebBackendTableHeaderOptions.getRowOptions();
            if (otherRowOptions != null)
                setRowOptions(otherRowOptions);

        }
    }

    public ComponentCreator<? extends CoreTableHeaderRow, ? extends CoreTableHeaderRowOptions> getRowCreator() {
        return rowCreator;
    }

    public S setRowCreator(ComponentCreator<? extends WebBackendTableHeaderRow, ? extends WebBackendTableHeaderRowOptions> rowCreator) {
        this.rowCreator = rowCreator;
        return self();
    }

    protected WebBackendTableHeaderRowOptions createDefaultRowOptions() {
        WebBackendTableHeaderRowOptions options = ((WebBackendTableHeaderRowOptions) (super.createDefaultRowOptions()));
        return options;
    }
}