...
import dev.reuise.web.core.graphics.svg.DefaultWebSvgPart;
...
public abstract class DefaultWebBackendSvg<S extends DefaultWebBackendSvg<S, O>, O extends WebBackendSvgOptions> extends DefaultWebSvgPart<S, O> implements WebBackendComponentType<S, O> , WebBackendSvg {
    private String replaceIds(String svg) {
        String docId = getUniqueId();
        svg = svg.replace("id=\"", "id=\"" + docId);
        svg = svg.replace("href=\"#", "href=\"#" + docId);
        svg = svg.replace("url(#", "url(#" + docId);
        return svg;
    }
    ...
    protected ClassResource sourceResource;

    protected DefaultWebBackendSvg(O options) {
        super(options);
    ...
    @Override
    public WebBackendSvg getComponent() {
        return ((WebBackendSvg) (baseComponentPart.getComponent()));
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        ...
        WebBackendSvgPart component = ((WebBackendSvgPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getSourceResourceOption(), component::setSourceResource);
    }
    ...
    public void loadFromResource(String path) {
        loadFromResource(path, null);
    }

    // Implementation
    public void loadFromResource(String path, Class<? extends Object> clazz) {
        this.sourceResource = new ClassResource(path, clazz);
    }
    ...
    public void writeTo(PrintWriter writer) {
        if (this.sourceResource != null) {
            ClassLoader classloader = (this.sourceResource.getClazz() != null) ? this.sourceResource.getClazz().getClassLoader() : Thread.currentThread().getContextClassLoader();
            try (InputStream in = classloader.getResourceAsStream(this.sourceResource.getPath())) {
                String contents = new String(in.readAllBytes(), StandardCharsets.UTF_8);
                writer.append(replaceIds(contents));
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
