...
import dev.reuise.web.core.progressindicator.AbstractWebProgressIndicatorOptions;
...
// Option: Debug - CORE
// Option: DebugId - CORE
public abstract class AbstractWebBackendProgressIndicatorOptions<S extends AbstractWebBackendProgressIndicatorOptions<S>> extends AbstractWebProgressIndicatorOptions<S> implements WebBackendProgressIndicatorOptions {
    ...
    protected ComponentCreator<? extends CoreContainer, ? extends CoreContainerOptions> trackCreator;

    protected AbstractWebBackendProgressIndicatorOptions() {
        this(null);
    }

    protected AbstractWebBackendProgressIndicatorOptions(WebBackendParentComponentOptions baseOptions) {
    ...
    public boolean onPreInitialize() {
        ...
        setTagName("div");
        setBaseStyleClass("reuise-progressindicator");
        return true;
    }
    ...
    @Override
    public WebBackendProgressIndicator getComponent() {
        return ((WebBackendProgressIndicator) (baseComponentPart.getComponent()));
    }

    @Override
    public void merge(CoreComponentOptions other) {
        baseComponentPart.merge(other);
        if (((WebBackendProgressIndicatorOptions) (other)) instanceof WebBackendProgressIndicatorOptions) {
            WebBackendProgressIndicatorOptions otherWebBackendProgressIndicatorOptions = ((WebBackendProgressIndicatorOptions) (other));
            CoreContainerOptions otherTrackOptions = otherWebBackendProgressIndicatorOptions.getTrackOptions();
            if (otherTrackOptions != null)
                setTrackOptions(otherTrackOptions);

        }
    }

    public ComponentCreator<? extends CoreContainer, ? extends CoreContainerOptions> getTrackCreator() {
        return trackCreator;
    }

    public S setTrackCreator(ComponentCreator<? extends WebBackendContainer, ? extends WebBackendContainerOptions> trackCreator) {
        this.trackCreator = trackCreator;
        return self();
    }

    protected WebBackendContainerOptions createDefaultTrackOptions() {
        WebBackendContainerOptions options = ((WebBackendContainerOptions) (super.createDefaultTrackOptions()));
        return options;
    }
}