...
import dev.reuise.web.core.tabs.AbstractWebTabOptions;
...
// Option: Debug - CORE
// Option: DebugId - CORE
public abstract class AbstractWebBackendTabOptions<S extends AbstractWebBackendTabOptions<S>> extends AbstractWebTabOptions<S> implements WebBackendTabOptions {
    ...
    protected ComponentCreator<? extends CoreBadge, ? extends CoreBadgeOptions> badgeCreator;

    protected ComponentCreator<? extends CoreIcon, ? extends CoreIconOptions> iconCreator;

    protected ComponentCreator<? extends CoreInlineText, ? extends CoreInlineTextOptions> labelContainerCreator;

    protected AbstractWebBackendTabOptions() {
        this(null);
    }

    protected AbstractWebBackendTabOptions(WebBackendParentComponentOptions baseOptions) {
    ...
    @Override
    public WebBackendTab getComponent() {
        return ((WebBackendTab) (baseComponentPart.getComponent()));
    }

    @Override
    public void merge(CoreComponentOptions other) {
        baseComponentPart.merge(other);
        if (((WebBackendTabOptions) (other)) instanceof WebBackendTabOptions) {
            WebBackendTabOptions otherWebBackendTabOptions = ((WebBackendTabOptions) (other));
            CoreLinkOptions otherLinkOptions = otherWebBackendTabOptions.getLinkOptions();
            if (otherLinkOptions != null)
                setLinkOptions(otherLinkOptions);

            CoreBadgeOptions otherBadgeOptions = otherWebBackendTabOptions.getBadgeOptions();
            if (otherBadgeOptions != null)
                setBadgeOptions(otherBadgeOptions);

            CoreIconOptions otherIconOptions = otherWebBackendTabOptions.getIconOptions();
            if (otherIconOptions != null)
                setIconOptions(otherIconOptions);

            CoreInlineTextOptions otherLabelContainerOptions = otherWebBackendTabOptions.getLabelContainerOptions();
    ...
    public ComponentCreator<? extends CoreBadge, ? extends CoreBadgeOptions> getBadgeCreator() {
        return badgeCreator;
    }

    public S setBadgeCreator(ComponentCreator<? extends WebBackendBadge, ? extends WebBackendBadgeOptions> badgeCreator) {
        this.badgeCreator = badgeCreator;
        return self();
    }

    protected WebBackendBadgeOptions createDefaultBadgeOptions() {
        WebBackendBadgeOptions options = ((WebBackendBadgeOptions) (super.createDefaultBadgeOptions()));
        return options;
    }

    public ComponentCreator<? extends CoreIcon, ? extends CoreIconOptions> getIconCreator() {
        return iconCreator;
    }

    public S setIconCreator(ComponentCreator<? extends WebBackendIcon, ? extends WebBackendIconOptions> iconCreator) {
        this.iconCreator = iconCreator;
        return self();
    }

    protected WebBackendIconOptions createDefaultIconOptions() {
        WebBackendIconOptions options = ((WebBackendIconOptions) (super.createDefaultIconOptions()));
        return options;
    }
