...
import dev.reuise.web.core.tabs.DefaultWebTabBarPart;
...
public abstract class DefaultWebBackendTabBar<S extends DefaultWebBackendTabBar<S, O>, O extends WebBackendTabBarOptions> extends DefaultWebTabBarPart<S, O> implements WebBackendComponentType<S, O> , WebBackendTabBar {
    @Override
    public S add(CoreComponent child) {
        if (child instanceof CoreTab)
            return addTab(((CoreTab) (child)));
    ...
    public void remove(CoreComponent child) {
        if (child instanceof CoreTab)
            removeTab(((CoreTab) (child)));

        this.parentComponentPart.remove(child);
    }
    ...
    protected DefaultWebBackendTabBar(O options) {
        super(options);
    ...
    @Override
    public WebBackendContainer getActiveIndicator() {
        return ((WebBackendContainer) (super.getActiveIndicator()));
    }
