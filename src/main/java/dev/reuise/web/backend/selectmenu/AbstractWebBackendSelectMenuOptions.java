...
import dev.reuise.web.core.selectmenu.AbstractWebSelectMenuOptions;
...
// Option: Debug - CORE
// Option: DebugId - CORE
public abstract class AbstractWebBackendSelectMenuOptions<S extends AbstractWebBackendSelectMenuOptions<S>> extends AbstractWebSelectMenuOptions<S> implements WebBackendSelectMenuOptions {
    ...
    protected ComponentCreator<? extends CoreTextField, ? extends CoreTextFieldOptions> textFieldCreator;

    protected ComponentCreator<? extends CoreMenu, ? extends CoreMenuOptions> menuCreator;

    protected AbstractWebBackendSelectMenuOptions() {
        this(null);
    }

    protected AbstractWebBackendSelectMenuOptions(WebBackendParentComponentOptions baseOptions) {
    ...
    public boolean onPreInitialize() {
        ...
        setTagName("div");
        setBaseStyleClass("reuise-selectmenu");
        return true;
    }
    ...
    @Override
    public WebBackendSelectMenu getComponent() {
        return ((WebBackendSelectMenu) (baseComponentPart.getComponent()));
    }

    @Override
    public void merge(CoreComponentOptions other) {
        baseComponentPart.merge(other);
        if (((WebBackendSelectMenuOptions) (other)) instanceof WebBackendSelectMenuOptions) {
            WebBackendSelectMenuOptions otherWebBackendSelectMenuOptions = ((WebBackendSelectMenuOptions) (other));
            CoreTextFieldOptions otherTextFieldOptions = otherWebBackendSelectMenuOptions.getTextFieldOptions();
            if (otherTextFieldOptions != null)
                setTextFieldOptions(otherTextFieldOptions);

            CoreMenuOptions otherMenuOptions = otherWebBackendSelectMenuOptions.getMenuOptions();
    ...
    public ComponentCreator<? extends CoreTextField, ? extends CoreTextFieldOptions> getTextFieldCreator() {
        return textFieldCreator;
    }

    public S setTextFieldCreator(ComponentCreator<? extends WebBackendTextField, ? extends WebBackendTextFieldOptions> textFieldCreator) {
        this.textFieldCreator = textFieldCreator;
        return self();
    }

    protected WebBackendTextFieldOptions createDefaultTextFieldOptions() {
        WebBackendTextFieldOptions options = ((WebBackendTextFieldOptions) (super.createDefaultTextFieldOptions()));
        return options;
    }
